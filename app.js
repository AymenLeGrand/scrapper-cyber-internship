...
function setupEventListeners() {
  ...
  const closeScrapeNotif = document.getElementById('closeScrapeNotif');
  if (closeScrapeNotif) {
    closeScrapeNotif.addEventListener('click', () => {
      document.getElementById('scrapeNotification').classList.add('hidden');
    });
  }
}
...
async function triggerScrape() {
  ...
  const notif = document.getElementById('scrapeNotification');
  const notifTitle = document.getElementById('scrapeStatusTitle');
  const notifDesc = document.getElementById('scrapeStatusDesc');
  const notifIcon = document.getElementById('scrapeStatusIcon');

  // Set loading state on button
  if (scrapeBtn) scrapeBtn.disabled = true;
  if (scrapeText) scrapeText.textContent = 'Scraping en cours...';
  if (scrapeIcon) {
    scrapeIcon.classList.add('spinning');
    scrapeIcon.classList.add('animate-spin');
  }

  // Display in-progress notification banner
  if (notif) {
    notif.className = 'rounded-xl border border-indigo-200 bg-indigo-50/90 p-4 transition-all duration-300 shadow-sm';
    if (notifIcon) notifIcon.textContent = '⚡';
    if (notifTitle) notifTitle.textContent = 'Scraping et vérification en cours...';
    if (notifDesc) notifDesc.textContent = 'Interrogation des ATS officiels (Synacktiv, Wavestone, Sopra Steria, HeadMind, VINCI, OCTO, Sia, Mazars, Devoteam), audit liveness HTTP 200 et filtre strict Zero-CDI...';
  ...
  try {
    ...
    if (notif) {
      notif.className = 'rounded-xl border border-rose-200 bg-rose-50/90 p-4 transition-all duration-300 shadow-sm';
      if (notifIcon) notifIcon.textContent = '⚠️';
      if (notifTitle) notifTitle.textContent = 'Serveur local de scraping requis';
      if (notifDesc) notifDesc.textContent = `En local, lancez 'python server.py' pour activer le scraping interactif par bouton web. (Détail: ${err.message})`;
    }
  } finally {
    if (scrapeBtn) scrapeBtn.disabled = false;
    if (scrapeText) scrapeText.textContent = 'Scraper maintenant';
    if (scrapeIcon) {
